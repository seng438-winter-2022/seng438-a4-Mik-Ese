...
public class DataUtilitiesTest{
  ...
  // equal
	...
	public void checkTwoArraysNotEqualTwice() {
		double array[][] = {	
				{1,2,3,4,5}
		};
		double array2[][] = {
    ...
    public void calculateColumnTotalAllPosIntTwice(){
        mockClass.checking(new Expectations(){ {
            atLeast(1).of(table).getRowCount();
            will(returnValue(3));

            atLeast(1).of(table).getValue(0, 0);
            will(returnValue(1));   // Row 0 column 0 has value 1

            atLeast(1).of(table).getValue(1, 0);
            will(returnValue(2));   // Row 1 column 0 has value 2

            atLeast(1).of(table).getValue(2, 0);
            will(returnValue(3));   // Row 2 column 0 has value 3
        } });

        DataUtilities.calculateColumnTotal(table, 0);
        assertEquals("calculateColumnTotal should return 6.0", DataUtilities.calculateColumnTotal(table, 0),
                6.0, 0.000000001d);
    }
    ...
    @Test(expected = IllegalArgumentException.class)	// mutation testing
	public void calculateRowTotalNullException() {
		DataUtilities.calculateRowTotal(null, 0);
	}
    
    @Test	// mutation testing
    public void calculateRowTotalAllPosIntTwice(){
        mockClass.checking(new Expectations(){ {
            atLeast(1).of(table).getColumnCount();
            will(returnValue(2));

            atLeast(1).of(table).getValue(0, 0);
            will(returnValue(55));   // Row 0 column 0 has value 55

            atLeast(1).of(table).getValue(0, 1);
            will(returnValue(55));   // Row 0 column 1 has value 55
        } });
    ...
    public void calculateRowTotalShouldReturnTwoTwice(){
        ...
        DataUtilities.calculateRowTotal(table, 0, validRows);
        assertEquals("calculateRowTotal should return 2.0", DataUtilities.calculateRowTotal(table, 0, validRows),
                2.0, 0.000000001d);
    }
	...
	@Test	// mutation testing
	public void createNumberArrayTestTwice() {
		Number array[] = new Number[2];
		double double1 = 1;
		double double2 = 2;
		array[0] = double1;
	...
	@Test	// mutation testing
	public void cumulativePercentageAtZeroShouldBeTenPercentTwice() {
	...
	@Test	// mutation testing
	public void calculateColumnTotalThreeParamsOfFirstAndThirdRowTwice() {
	    mockClass.checking(new Expectations(){ {
		    atLeast(1).of(table).getRowCount();
		    will(returnValue(3));
		...
		DataUtilities.calculateColumnTotal(table, column, validRows);
	    assertEquals("calculateColumnTotal(Values2D data, int column, int[] validRows) should return 4.0",
	    		4.0, DataUtilities.calculateColumnTotal(table, column, validRows), 0.000000001d);
	}
    ...
    @Test(expected = IllegalArgumentException.class)	// mutation testing
	public void create2DnumberArrayNullException() {
